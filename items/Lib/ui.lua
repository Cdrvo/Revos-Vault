...
SMODS.current_mod.custom_ui = function(modNodes)
		...
		nodes = {
			{
				...
				config = {
					object = DynaText({
						string = { "Revo's Vault" },
						colours = { SMODS.Gradients["crv_revosvault"] },
						shadow = true,
						float = true,
						spacing = 0,
						rotate = true,
	...
	modNodes[#modNodes + 1] = {
		...
		config = { align = "cm", padding = 0.07, no_fill = true },
		nodes = {
			{  n = G.UIT.T, config = { text = "by: CodeRevo", scale = 0.4, colour = G.C.PURPLE, shadow = true } },
		},
	}
end
...
function RevosVault.printer_box()
	...
	for i = 1, math.ceil(#printers / (5 * #G.your_collection)) do
		table.insert(
			joker_options,
			localize("k_page") .. " " .. tostring(i) .. "/" .. tostring(math.ceil(#printers / (5 * #G.your_collection)))
		)
	end
			...
			{ n = G.UIT.R, config = { align = "cm", r = 0.1, colour = G.C.BLACK, emboss = 0.05 }, nodes = deck_tables },
			{
				...
				nodes = {
					create_option_cycle({
						...
						opt_callback = "your_collecion_printer_list",
						current_option = 1,
						colour = G.C.RED,
						no_pips = true,
...
RevosVault.button_func = function(card, args) -- idk what im doing
	...
	if not args.colour then
		args.colour = G.C.RED
	end
	if not args.one_press then
		args.one_press = false
	end
	if not args.text_colour then
		args.text_colour = G.C.UI.TEXT_LIGHT
	end
	if not args.text_scale then
		args.text_scale = 0.5
	end
	if not args.r then
		args.r = 0.08
	end
	if not args.minw then
		args.minw = 1.25
	end
	if not args.minh then
		args.minh = 0
	end
	if not args.padding_1 then
		args.padding_1 = 0.1
	end
	if not args.align_after_text then
		args.align_after_text = "cm"
	end
	if not args.ref_table then
		args.ref_table = card
	end
	if not args.hover then
		args.hover = true
	end
	if not args.shadow then
		args.shadow = true
	end
	if not args.shadow_text then
		args.shadow_text = true
	end
	if not args.maxw then
		args.maxw = 1.25
	end
	if not args.maxw_after_text then
		args.maxw_after_text = 1.25
	end
...
function Card:highlight(is_highlighted)
	...
	elseif
		...
		and self.config.center.key == "j_crv_thed6"
		and self.area == G.jokers
	then
		if self.children.use_button then
			self.children.use_button:remove()
	...
	elseif
		...
		and self.config.center
		and self.config.center.key
		and self.config.center.key == "j_crv_brj"
		and self.area == G.jokers
	then
		if self.children.use_button then
			self.children.use_button:remove()
	...
	elseif
		...
		and self.config.center
		and self.config.center.key
		and self.config.center.key == "j_crv_invest"
		and self.area == G.jokers
	then
		if self.children.use_button then
			self.children.use_button:remove()
	...
	elseif
		...
		and self.config.center
		and self.config.center.key
		and self.config.center.key == "j_crv_roulj"
		and self.area == G.jokers
	then
		if self.children.use_button then
			self.children.use_button:remove()
	...
	elseif
		...
		and self.config.center
		and self.config.center.key
		and self.config.center.key == "j_crv_dealb"
		and self.area == G.jokers
	then
		if self.children.use_button then
			self.children.use_button:remove()
...
function RevosVault.localize(args, misc_cat)
	...
	if args.stylize then
		loc_target = RevosVault.manual_parse(loc_target)
	end
			...
			for _, lines in
				ipairs(
					args.type == "unlocks" and loc_target.unlock_parsed
						...
						or args.type == "text" and loc_target
		...
		if loc_target.multi_line then
			...
			for k, v in ipairs(loc_target) do
				...
				for _, subpart in ipairs(v[1].strings) do
					assembled_string = assembled_string
						.. (type(subpart) == "string" and subpart or format_ui_value(args.vars[tonumber(subpart[1])]))
				end
	...
	elseif args.type == "name_text" then
		if
			pcall(function()
				local name_text = (loc_target and loc_target.name)
					or G.localization.descriptions[(args.set or args.node.config.center.set)][args.key or args.node.config.center.key].name
	...
	elseif args.type == "name" then
		...
		if
			pcall(function()
				local name = loc_target
					or G.localization.descriptions[(args.set or args.node.config.center.set)][args.key or args.node.config.center.key]
	...
	if ret_string and type(ret_string) == "string" then
	...
	if loc_target then
		...
		if
			(args.type == "descriptions" or args.type == "other")
			and type(loc_target.text) == "table"
			and type(loc_target.text[1]) == "table"
		then
			args.AUT.multi_box = {}
			for i, box in ipairs(loc_target.text_parsed) do
				for j, line in ipairs(box) do
					...
					if not next(args.AUT.info) then
						args.AUT.box_colours[i] = args.vars.box_colours and args.vars.box_colours[i]
							or G.C.UI.BACKGROUND_WHITE
					end
				end
			end
			return
		...
		do
			...
			if args.type == "name" and loc_target.name_parsed then
				for _, part in ipairs(lines) do
					...
					for _, subpart in ipairs(part.strings) do
						assembled_string_part = assembled_string_part
							.. (
								type(subpart) == "string" and subpart
								or format_ui_value(format_ui_value(args.vars[tonumber(subpart[1])]))
								or "ERROR"
							)
					end
			...
			for _, part in ipairs(lines) do
				...
				if args.type == "name" then
					final_line[#final_line + 1] = {
						...
						config = {
							align = "m",
							colour = part.control.B and args.vars.colours[tonumber(part.control.B)]
								or part.control.X and loc_colour(part.control.X)
								or nil,
							r = 0.05,
							padding = 0.03,
							res = 0.15,
					...
					final_line[#final_line].nodes[1] = {
						...
						config = {
							object = DynaText({
								...
								spacing = (not args.no_spacing and (args.spacing or 1) * math.max(
									0,
									0.32 * (17 - #(final_name_assembled_string or assembled_string))
								)) or nil,
								font = SMODS.Fonts[part.control.f]
									or G.FONTS[tonumber(part.control.f)]
									or (SMODS.Fonts[args.font] or G.FONTS[args.font]),
								scale = (0.55 - 0.004 * #(final_name_assembled_string or assembled_string))
									* (part.control.s and tonumber(part.control.s) or args.scale or 1)
									* (args.fixed_scale or 1),
							}),
						},
					}
					...
					final_line[#final_line + 1] = {
						...
						config = {
							detailed_tooltip = part.control.T
									and (G.P_CENTERS[part.control.T] or G.P_TAGS[part.control.T] or G.DetailedTooltips[part.control.T])
								or nil,
							text = assembled_string,
							font = SMODS.Fonts[part.control.f]
								or G.FONTS[tonumber(part.control.f)]
			...
			if not args.nodes and args.type == "name" then
				args.nodes = {}
			end
...
function RevosVault_create_info_UI(args)
												...
												nodes = {
													{
														...
														config = {
															...
															focus_args = not args.no_pip
																	and { button = args.back_button or "b" }
																or nil,
														},
													},
												},
	...
	if loc and loc.text and #loc.text > 1 then
		local pages = {
			{
				...
				config = {
					...
					colour = G.C.BLACK,
					shadow = true,
					button = "RevosVault_previous_info_page",
					menu_type = menu_type,
					page = page,
			...
			},
			{
				...
				config = {
					...
					colour = G.C.BLACK,
					shadow = true,
					button = "RevosVault_next_info_page",
					menu_type = menu_type,
					page = page,
					max_page = (#(loc.text or {}) or 1),
				},
		...
		for _, v in ipairs(pages) do
			ret.nodes[1].nodes[1].nodes[1].nodes[1].nodes[4].nodes[#ret.nodes[1].nodes[1].nodes[1].nodes[1].nodes[4].nodes + 1] =
				v
		end
	end
...
G.FUNCS.RevosVault_info = function(args)
	if not args or not args.menu_type or not G.localization.Menus[args.menu_type] then
		return
	end
